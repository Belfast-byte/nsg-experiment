// index_nsg.cpp — OPTIMIZED v2: Version Array only (NO heap)
// Keeps original sorted-array InsertIntoPool for stability.
// Key optimization: replaces boost::dynamic_bitset with visited_/search_id_ 
// (zero heap allocation per query).
...
IndexNSG::~IndexNSG() {
    if (distance_ != nullptr) { delete distance_; distance_ = nullptr; }
    if (initializer_ != nullptr) { delete initializer_; initializer_ = nullptr; }
    if (opt_graph_ != nullptr) { free(opt_graph_); opt_graph_ = nullptr; }
}

// ── Save / Load / Load_nn_graph ── (unchanged from original)
...
void IndexNSG::get_neighbors(const float *query, const Parameters &parameter,
  ...
  for (unsigned i = 0; i < init_ids.size() && i < final_graph_[ep_].size(); i++) {
    ...
    flags[init_ids[i]] = true; L++;
  }
  while (L < init_ids.size()) {
    unsigned id = rand() % nd_;
    if (flags[id]) continue;
    init_ids[L] = id; L++; flags[id] = true;
  }
  L = 0;
  for (unsigned i = 0; i < init_ids.size(); i++) {
    ...
    retset[i] = Neighbor(id, dist, true); L++;
  }
...
// ── init_graph, sync_prune, InterInsert, Link, Build ── (unchanged)
...
void IndexNSG::init_graph(const Parameters &parameters) {
  ...
  for (unsigned j = 0; j < dimension_; j++) center[j] /= nd_;
...
void IndexNSG::sync_prune(unsigned q, std::vector<Neighbor> &pool,
  ...
  while (result.size() < range && (++start) < pool.size() && start < maxc) {
    ...
    for (unsigned t = 0; t < result.size(); t++) {
      if (p.id == result[t].id) { occlude = true; break; }
...
void IndexNSG::InterInsert(unsigned n, unsigned range,
  ...
  for (size_t i = 0; i < range; i++) {
    ...
    if (temp_pool.size() > range) {
      ...
      { LockGuard guard(locks[des]);
        for (unsigned t = 0; t < result.size(); t++) des_pool[t] = result[t]; }
    } else {
      LockGuard guard(locks[des]);
...
#pragma omp for schedule(dynamic, 100)
    for (unsigned n = 0; n < nd_; ++n) {
      pool.clear(); tmp.clear(); flags.reset();
...
void IndexNSG::Search(const float *query, const float *x, size_t K,
  ...
  for (size_t i = 0; i < K; i++) indices[i] = retset[i].id;
}

// ═══════════════════════════════════════════════════════════
// OPTIMIZED SearchWithOptGraph
// Change: Version Array (visited_/search_id_) replaces boost::dynamic_bitset
// Sorted-array InsertIntoPool preserved (same algorithm, zero-alloc visited)
// ═══════════════════════════════════════════════════════════
void IndexNSG::SearchWithOptGraph(const float *query, size_t K,
  ...
  // ── VERSION ARRAY: zero-heap-allocation visited tracking ──
  search_id_++;
  unsigned cur_id = search_id_;
  std::vector<unsigned>& visited = visited_;
  ...
  for (; tmp_l < L && tmp_l < MaxM_ep; tmp_l++) {
    ...
    visited[init_ids[tmp_l]] = cur_id;
  }
  while (tmp_l < L) {
    unsigned id = rand() % nd_;
    if (visited[id] == cur_id) continue;
    visited[id] = cur_id;
    init_ids[tmp_l] = id; tmp_l++;
  }
  ...
  while (k < (int)L) {
    ...
    if (retset[k].flag) {
      ...
      unsigned *nbrs = (unsigned *)(opt_graph_ + node_size * n + data_len);
      unsigned MaxM = *nbrs; nbrs++;
      for (unsigned m = 0; m < MaxM && m < 4; ++m)
        _mm_prefetch(opt_graph_ + node_size * nbrs[m], _MM_HINT_T0);
...
// ═══════════════════════════════════════════════════════════
// OptimizeGraph with 32-byte aligned layout
// ═══════════════════════════════════════════════════════════
void IndexNSG::OptimizeGraph(float *data) {
  data_ = data;
  // Align to 32 bytes (for _mm256_load_ps in future SIMD optimization)
  unsigned aligned_dim = (dimension_ + 1 + 7) & ~7U;
  data_len = aligned_dim * sizeof(float);
  neighbor_len = (width + 1) * sizeof(unsigned);
  node_size = data_len + neighbor_len;

  opt_graph_ = (char *)aligned_alloc(32, node_size * nd_);
  if (!opt_graph_) {
  ...
  for (unsigned i = 0; i < nd_; i++) {
    ...
    if (aligned_dim > dimension_ + 1) {
      size_t pad_start = sizeof(float) + dimension_ * sizeof(float);
      size_t pad_size = data_len - pad_start;
      memset(cur_node_offset + pad_start, 0, pad_size);
    }
...
// ── DFS, findroot, tree_grow (unchanged) ──
  ...
  while (!s.empty()) {
    ...
    for (unsigned i = 0; i < final_graph_[tmp].size(); i++) {
      if (flag[final_graph_[tmp][i]] == false) {
        next = final_graph_[tmp][i]; break;
      }
    }
    if (next == (nd_ + 1)) { s.pop(); if (s.empty()) break; tmp = s.top(); continue; }
    tmp = next; flag[tmp] = true; s.push(tmp); cnt++;
  }
}
...
void IndexNSG::findroot(boost::dynamic_bitset<> &flag, unsigned &root,
  ...
  for (unsigned i = 0; i < pool.size(); i++) {
    if (flag[pool[i].id]) { root = pool[i].id; found = 1; break; }
  }
  if (found == 0) {
    while (true) {
      unsigned rid = rand() % nd_;
      if (flag[rid]) { root = rid; break; }
    }
  }
...
void IndexNSG::tree_grow(const Parameters &parameter) {
  ...
  for (size_t i = 0; i < nd_; ++i) {
    if (final_graph_[i].size() > width) width = final_graph_[i].size();
  }
}
}
